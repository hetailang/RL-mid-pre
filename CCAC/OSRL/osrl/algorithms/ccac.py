...
class CCAC(nn.Module):
    ...
    def _classifier_loss(self, probs, labels):
        ...
        if self.classifier_loss == "focal":
            bce = F.binary_cross_entropy(probs, labels, reduction="none")
            p_t = torch.where(labels > 0.5, probs, 1.0 - probs)
            alpha_t = torch.where(labels > 0.5, self.focal_alpha, 1.0 - self.focal_alpha)
            return (alpha_t * (1.0 - p_t).pow(self.focal_gamma) * bce).mean()
    ...
    def vae_loss(self, observations, actions, cost_thresholds):
        ...
        loss_class = self._classifier_loss(logits, labels)
    ...
    def cost_critic_loss_cvae(self, observations, next_observations, actions, 
        ...
        with torch.no_grad():
            ...
            ood_scores = self.classifier.forward(gen_sac)
            if self.ood_mode == "none":
                ood_weights = torch.ones_like(ood_scores)
            elif self.ood_mode == "soft":
                ood_weights = ood_scores.clamp(0.0, 1.0)
            elif self.ood_mode == "hard":
                ood_weights = (ood_scores >= self.ood_threshold).float()
            else:
                raise ValueError(f"Unsupported ood_mode: {self.ood_mode}")
            ood_mask = ood_weights > 0
            ood_sa = gen_sa[ood_mask, :]
            ood_cost = cost_thresholds[ood_mask]
            ood_weights = ood_weights[ood_mask]
        ...
        if torch.sum(ood_mask) > 0:
            ...
            loss_lag1 = - ood_weights * lmbda1_detach * (qc_ood - ood_cost)
            ...
            # update lagrangian
            loss_lag1 = ood_weights * lmbda1 * (qc_ood_detach - ood_cost)
